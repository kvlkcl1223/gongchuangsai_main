                ...
                while count < 5:
                    ...
                    if index_garbage <= 100000000000000:
                        if len(cls_) == 1:
                            ...
                            time.sleep(0.1)
                            ret, frame = cap.read()
                            new_cls_, confs, _, angles, centers, image, new_areas = model(frame,
                                                                                      conf_threshold=conf_threshold,
                                                                                      iou_threshold=0.5)
                            # 判断是否识别正确
                            ...
                            if new_cls_ == cls_ and abs(new_areas[0] - areas[0]) < 0.1:
                                ...
                                command = f'Tar='
                                command_display = f''
                                center = transform_point_to_rotated_coords_clockwise(centers[0])
                                # 有害垃圾
                                if cls_[0] == 1 or cls_[0] == 2 or cls_[0] == 8:
                                    command += f'j1x{center[0]}y{center[1]}a105!'
                                    command_display += 'harmful=!'
                                # 可回收垃圾
                                elif cls_[0] == 5 or cls_[0] == 9:
                                    command += f'j2x{center[0]}y{center[1]}a105!'
                                    command_display += 'recycle=!'
                                # 厨余垃圾
                                elif cls_[0] == 3 or cls_[0] == 7:
                                    command += f'j3x{center[0]}y{center[1]}a105!'
                        ...
                        if len(cls_) == 2:
                            ...
                            time.sleep(0.1)
                            ret, frame = cap.read()
                            new_cls_, confs, _, angles, centers, image, areas = model(frame,
                                                                                      conf_threshold=conf_threshold,
                                                                                      iou_threshold=0.5)
                            # 认为识别正确
